...
#include "SW.h"
...
#if defined(SW__PC)
    #define SW_SetP4PinDriveMode(shift, mode)  \
    do { \
        SW_PC =   (SW_PC & \
                                (uint32)(~(uint32)(SW_DRIVE_MODE_IND_MASK << \
                                (SW_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (SW_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define SW_SetP4PinDriveMode(shift, mode)  \
        do { \
            SW_USBIO_CTRL_REG = (SW_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(SW_DRIVE_MODE_IND_MASK << \
                                    (SW_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (SW_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		SW_SetP4PinDriveMode(SW__0__SHIFT, mode);
    }
#endif
...
void SW_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  SW_INTCFG & (uint32)(~(uint32)position);
    SW_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
