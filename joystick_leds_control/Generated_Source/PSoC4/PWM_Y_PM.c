...
static PWM_Y_BACKUP_STRUCT PWM_Y_backup;
...
{
    if(0u != (PWM_Y_BLOCK_CONTROL_REG & PWM_Y_MASK))
    {
        PWM_Y_backup.enableState = 1u;
    }
    else
    {
        PWM_Y_backup.enableState = 0u;
    }
...
void PWM_Y_Wakeup(void)
{
    ...
    if(0u != PWM_Y_backup.enableState)
    {
        PWM_Y_Enable();
    }
}
